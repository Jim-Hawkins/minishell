...
void mycalc(char *command[], char *res, int num_args){
	...
	if(num_args != 4){
		sprintf(res, "[ERROR] La estructura del comando es <operando 1> <add/mod> <operando 2>\n");
				...
				case 0:  // son
					...
					if( strcmp(filev[0], "0") != 0){			//first of all, check possible redirections,
						if(close(STDIN_FILENO) < 0){
							perror("Error:");
							exit(-1);							
							}				//  close the corresponding entries of the BCP table
				...
				default:	//FATHER'S CODE
					if(i != (command_counter - 1)){		//in every iteration, except the last one, the father process saves the output of
						...
						if(close(last_fid) < 0){
							perror("Error:");
							exit(-1);
						}			//  (it would make no difference closing p[0] or last_fid, but this way is clearer)
					}
